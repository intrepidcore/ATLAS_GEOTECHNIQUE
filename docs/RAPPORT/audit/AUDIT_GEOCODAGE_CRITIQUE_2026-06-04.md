...
## 16. Implémentation de la Roadmap — Journal des Livraisons (2026-06-04)

### 16.1 Fichiers Créés / Modifiés dans cette Session

| Fichier | Type | Description |
|---------|------|-------------|
| `migrations/104_fix_geocode_sondage_location_mode.sql` | Migration SQL | Fix `geocode_sondage()` — location_mode (Bug #2) |
| `migrations/105_geocode_suggestions_extended.sql` | Migration SQL | Table `atlas.geocode_suggestions` + contrainte anti-fallback (Prop. 3) |
| `scripts/regeocod_fuzzy_v1.py` | Script Python | Re-géocodage fuzzy-regex (Section 4.5) — Phase 2 |
| `scripts/run_all_models_nightly_v2.py` | Script Python | Orchestrateur nightly v2 via API Rust (gain de perf) |
| `.github/workflows/ml-pipeline.yml` | CI/CD | Ajout job `db-coherence-tests` — 4 tests SQL (Section 11) |

### 16.2 Documents Rédigés / Mis à Jour

| Document | Statut | Contenu |
|----------|--------|---------|
| `docs/RECHERCHE/RESULTATS_FUSION_KED_RK.md` | ✅ Mis à jour | Alerte provisoire + Section 7 (actions post-correction + script) |
| `docs/RECHERCHE/RESULTATS_MTGP_BLOC_D.md` | ✅ Mis à jour | Alerte provisoire + Section 7 (plan relancement MTGP) |
| `docs/RECHERCHE/RESULTATS_VFS_BLOC_C.md` | ✅ Mis à jour | Section 7.5 (checklist de déblocage complète) |
| `docs/RAPPORT/TECHNIQUE_SCIENTIFIQUE/RAPPORT_FINAL_MODELES_GEOSTATS_2026-06-04.md` | ✅ Créé | Rapport daté post-audit : état, procédure, métriques baseline |
| `docs/RAPPORT/audit/AUDIT_GEOCODAGE_CRITIQUE_2026-06-04.md` (ce fichier) | ✅ Mis à jour | Section 16 — Journal des livraisons |

### 16.3 Orchestrateur Nightly v2 — Contexte

Le script `run_all_models_nightly.py` (v1) orchestre les modèles ML via `subprocess.run()`.
La nouvelle version `run_all_models_nightly_v2.py` passe par l'API Rust :

```
v1 : Python → subprocess.run(script.py) → PostgreSQL direct
v2 : Python → POST /ai/jobs/enqueue → Rust worker → PostgreSQL
```

**Gains de la v2** : mémoire libérée entre jobs, logs en DB + UI, SKIP LOCKED anti-doublon,
checkpoint DB persistant, GPU configuré au niveau worker Rust.

**Usage post-correction** :
```bash
# Après re-géocodage validé :
python scripts/run_all_models_nightly_v2.py 2>&1 | tee logs/nightly_$(date +%Y%m%d).log
```

### 16.4 Statut des Phases de la Roadmap

| Phase | Description | Statut |
|-------|-------------|--------|
| 0 | Correctifs immédiats (migration 102, fix UI) | ✅ Complété (session 2026-06-04) |
| 1 | Marquage sondages invalides (migration 103) | ✅ Complété (session 2026-06-04) |
| 2a | Migration 104 — fix `geocode_sondage()` | ✅ Script préparé, **à appliquer** |
| 2b | Migration 105 — table suggestions + contrainte | ✅ Script préparé, **à appliquer** |
| 2c | Script re-géocodage fuzzy `regeocod_fuzzy_v1.py` | ✅ Créé, **à exécuter** |
| 3 | Géocodage manuel via UI | ⬜ Après Phase 2 — intervention utilisateur |
| 4 | Relancement modèles L1→L2a→L2b→L3→L4 | ⬜ Après Phase 3 — `run_all_models_nightly_v2.py` |
| 5 | Mise à jour article scientifique | ⬜ Après Phase 4 |
| CI | Tests cohérence DB dans ml-pipeline.yml | ✅ Ajouté (job `db-coherence-tests`) |

### 16.5 Commandes de Démarrage (Ordre Recommandé)

```bash
# 1. Appliquer les migrations
PGPASSWORD=atlas psql -h 127.0.0.1 -p 5433 -U atlas -d atlas_clean \
  -f atlas_reclone/services/api-geo/migrations/104_fix_geocode_sondage_location_mode.sql
PGPASSWORD=atlas psql -h 127.0.0.1 -p 5433 -U atlas -d atlas_clean \
  -f atlas_reclone/services/api-geo/migrations/105_geocode_suggestions_extended.sql

# 2. Re-géocodage fuzzy dry-run (vérification)
cd atlas_reclone
python scripts/regeocod_fuzzy_v1.py --dry-run
# Analyser logs/regeocode_fuzzy_*_dryrun.csv

# 3. Re-géocodage fuzzy réel
python scripts/regeocod_fuzzy_v1.py

# 4. Valider manuellement les suggestions dans l'UI http://localhost:1420

# 5. Lancer les modèles ML (v2 via API Rust)
python scripts/run_all_models_nightly_v2.py
```


## 17. Correction requise dans l'article scientifique — Hiérarchie KED-H

**Découvert le** : 2026-06-05 — vérification croisée code source vs article  
**Vérification par** : lecture directe de `scripts/run_ked_vbs_ip_wl_wp_horizons.py` (lignes 274–321) et `scripts/sql/create_contexte_geologique.sql` (lignes 34–40)

### 17.1 Divergence identifiée

L'article décrit en Équation 4 l'ordre de priorité suivant pour la dérive hiérarchique KED-H :

| Niveau article | Critère |
|---|---|
...
**L'implémentation réelle dans le code suit un ordre différent**, tel que défini dans `contexte_complet` :

```sql
-- create_contexte_geologique.sql, lignes 35-40
CONCAT_WS('|',
    COALESCE(z.nom, 'NAT'),                    -- seg.1 : Zone géomorphologique
    LEFT(COALESCE(up.type_sol, 'INC'), 4),     -- seg.2 : Pédologie
    COALESCE(rg.niveau_risque, 'INC'),         -- seg.3 : Risque RGA
    LEFT(COALESCE(ug.type_sols, 'INC'), 6)     -- seg.4 : Formation géologique
) AS contexte_complet
```

Et le repli Python (`compute_hierarchical_prior`, `min_pts=5`) :

| Niveau code | Critère retenu | Critère abandonné |
|---|---|---|
| 1 | ZONE \| PEDO \| RISQUE \| GEO | — |
| 2 | ZONE \| PEDO | RISQUE + GEO |
| 3 | PEDO seule | ZONE + RISQUE + GEO |
| 4 | Moyenne nationale | tout |

### 17.2 Conséquence pratique

Dans le code, **RGA n'est discriminant qu'au niveau 1** (contexte complet). Dès le repli niveau 2, RGA est abandonné. Cela signifie que RGA agit comme un discriminant fin à l'intérieur d'une zone/pédologie donnée, non comme le filtre primaire de l'entonnoir comme le décrit l'article.

La zone géomorphologique (Dépression de la Lama, Plaines côtières, etc.) est en pratique le **premier prédicteur structurant** au Togo, ce qui justifie son placement en niveau 1 dans le code — mais cette décision de conception n'est pas explicitée dans l'article.

### 17.3 Correction à apporter dans l'article

**Section à corriger** : Équation 4 et commentaire associé.

**Texte actuel (article)** :
> Niveau de priorité : RGA (1) → Zone géomorphologique (2) → Pédologie (3) → Géologie (4) → Nationale (5)

**Texte corrigé à proposer** :
> L'implémentation retient l'ordre : Zone géomorphologique (1) → Pédologie (2) → [Zone × Pédologie × RGA × Géologie] pour le contexte complet, avec repli progressif vers la moyenne nationale si n_k < 5. Le RGA intervient uniquement dans le contexte le plus fin (niveau 1 du repli) ; il est abandonné dès le repli au niveau zone × pédologie. Ce choix de conception reflète le rôle structurant des zones géomorphologiques togolaises (Dépression de la Lama, Plaines côtières, Atakora) comme premier facteur de différenciation des propriétés géotechniques.

### 17.4 Action requise

- [ ] Mettre à jour l'Équation 4 de l'article pour refléter l'ordre réel du code
- [ ] Ou aligner le code sur l'article (réordonner `contexte_complet` → `RISQUE|ZONE|PEDO|GEO`) et adapter le repli Python
- [ ] Discuter avec les co-auteurs lequel des deux ordres est scientifiquement préférable

**Fichiers à modifier selon le choix :**
- Si on aligne l'article sur le code : section 3.2 de l'article + Équation 4
- Si on aligne le code sur l'article : `scripts/sql/create_contexte_geologique.sql` (lignes 35–40) + `scripts/run_ked_vbs_ip_wl_wp_horizons.py` (fonction `compute_hierarchical_prior`, lignes 299–319)

*Note : le seuil n_k ≥ 5 (confirmé dans le code) et la formulation "moyenne conditionnelle" sont corrects dans l'article.*

---

## 18. Mise à jour du statut — Session 2026-06-05

**Date de mise à jour** : 2026-06-05  
**Auteur** : Claude Sonnet 4.6 (IA assistante)

### 18.1 État du géocodage au 2026-06-05

```sql
-- Résultat de vérification DB port 5433 le 2026-06-05
SELECT is_geocoded, location_mode, COUNT(*)
FROM atlas.sondages WHERE deleted_at IS NULL
GROUP BY is_geocoded, location_mode;

-- RÉSULTAT :
-- is_geocoded=true  : 572 sondages (100%)
-- Sondages sans géométrie : 0 (résolu)
-- Mode Géocodage Manuel "Sondages sans géométrie" dans l'UI : ✅ 0
```

```sql
-- Statut des suggestions de géocodage
SELECT COUNT(*), status FROM public.geocode_suggestions GROUP BY status;
-- count | status
-- ------+--------
--     6 | pending     ← 6 restantes à valider manuellement
--   194 | accepted    ← 194 appliquées aux sondages
```

| Indicateur | Valeur au 04/06 | Valeur au 05/06 | Évolution |
|---|---|---|---|
| Sondages sans géométrie | 187 (POINT 1,8.6) | 0 | ✅ Résolu |
| Suggestions pending | ~200 | **6** | ✅ 194 validées |
| Suggestions accepted | 0 | **194** | ✅ Appliquées |
| Sondages totaux géocodés | ~385 | **572** | ✅ 100% |
| `adm_random_cell` | 445 | 445 | Stable (approximatif) |
| `exact` | 127 | 127 | Stable (GPS réels) |

**Synthèse** : Les Phases 2 (fuzzy) et 3 (manuel) sont **terminées**. Les 200 suggestions générées par le script `regeocod_fuzzy_v1.py` ont été traitées : 194 acceptées via l'interface UI, 6 restantes (en attente de validation manuelle — sondages avec noms de localités introuvables dans ADM3).

### 18.2 Statut des Phases de la Roadmap au 2026-06-05

| Phase | Description | Statut au 04/06 | Statut au 05/06 |
|-------|-------------|-----------------|-----------------|
| 0 | Correctifs immédiats (migration 102, fix UI) | ✅ Complété | ✅ Complété |
| 1 | Marquage sondages invalides (migration 103) | ✅ Complété | ✅ Complété |
| 2a | Migration 104 — fix `geocode_sondage()` | À appliquer | ✅ Appliqué |
| 2b | Migration 105 — table suggestions | À appliquer | ✅ Appliqué |
| 2c | Re-géocodage fuzzy `regeocod_fuzzy_v1.py` | À exécuter | **✅ Complété** (194/200 acceptés) |
| 3 | Géocodage manuel via UI | ⬜ Après Phase 2 | **✅ Complété** (6 restantes) |
| 4 | Relancement modèles L1→L2a→L2b→L3→L4 | ⬜ Après Phase 3 | **🔄 En cours** (session 05/06) |
| 5 | Mise à jour article scientifique | ⬜ Après Phase 4 | ⬜ Après finalisation Phase 4 |

### 18.3 Résultats des modèles ML — Session 2026-06-05

Les modèles ont été relancés sur les 572 sondages correctement géocodés :

| Modèle | Paramètres | Horizons | Status | Métriques clés |
|--------|-----------|----------|--------|----------------|
| KED-HIER | vbs, ip, wl, wp, eg | H1/H2/H3 | ✅ | — |
| RK SCORPAN | vbs, ip, wl, wp, eg | H1/H2/H3 | ✅ | 15/15 runs OK |
| BLUP Fusion | vbs, ip, wl, wp, eg | H1/H2/H3 | ✅ | 47-49% réduction variance |
| MTGP | plasticite, compactage | H1/H2/H3 | ✅ | 441,105 + 88,221 valeurs |
| VfS Sentinel-2 | — | — | ⬜ | Bloqué (GEE + internet) |

**Note** : La BLUP Fusion (Rust `/ai/blup/fuse`) échoue avec `erreur de syntaxe près de VALUES` (bug `push_values` dans `blup_fusion.rs`). Fallback Python `ked_rk_fusion.py` utilisé avec succès. Correction UNNEST préparée dans `blup_fusion.rs` — rebuild requis (WiFi nécessaire pour `rust:1.86` manquant, ~200MB).

### 18.4 Paramètres V11 (CBR/Proctor/Pressiomètre/Pénétromètre)

Disponibilité en DB des données source :
- `essais_cbr` : 714 enregistrements → CBR_95 et γd disponibles
- `essais_proctor` : 224 enregistrements → γd_max et w_opt (**BUG corrigé le 05/06** : colonne `gamma_d_max` pas `rho_d_max`)
- `essais_pressiometre` : 26 enregistrements → Em, Pl
- `essais_penetrometre` : 411 enregistrements → Rd

Les paramètres V11 (cbr_95, gamma_d, w_opt, em_mpa, pl_mpa, rd_mpa) ne sont **pas encore calculés** via les modèles ML car :
1. RK SCORPAN ne couvre pas ces paramètres (pas de covariate SCORPAN pour la résistance)
2. KED-HIER pour H2/H3 de cbr_95/gamma_d/w_opt reste à lancer
3. BLUP fusion impossible sans RK comme second estimateur

Plan : relancer KED-HIER seul (pas de fusion possible) pour V11 une fois le rebuild disponible.

### 18.5 Améliorations UI déployées le 2026-06-05

La vue détail sondage (`sondages-list-panel.ts`) a été entièrement refactorisée :

1. **11 paramètres ML** : Affichés avec indicateurs visuels (✓ vert / — grisé)
2. **Catégories** : Atterberg, VBS, Granulométrie, Proctor, CBR, Pressiomètre, Pénétromètre, Classification, Gonflement, Physiques
3. **Courbe PSD** : SVG inline logarithmique pour la granulométrie (sieve_mm vs passing_pct)
4. **Profil CBR** : SVG barre horizontale (profondeur vs CBR%)
5. **Backend corrigé** : Requête Proctor fixée (`rho_d_max` → `gamma_d_max`), ajout CBR/pressiomètre/pénétromètre à `/sondages/:id/details` (rebuild WiFi requis)


---

## 19. Mise à jour du statut — Session 2026-06-07
## Revue directeur + analyse distribution spatiale + artefacts de correction

**Date** : 2026-06-07  
**Contexte** : Analyse complète de la distribution spatiale des sondages, revue critique du directeur de thèse, et identification des artefacts à créer pour la prochaine itération du manuscrit.

### 19.1 Roadmap complète au 2026-06-07

| Phase | Description | Statut |
|-------|-------------|--------|
| 0 | Fix vue couverture (migration 102) | ✅ **Complété** (06-04) |
| 1 | Marquage sondages fallback (migration 103) | ✅ **Complété** (06-04) |
| 2a | Fix `geocode_sondage()` location_mode (migration 104) | ✅ **Appliqué** (06-05) |
| 2b | Table suggestions + contrainte (migration 105) | ✅ **Appliqué** (06-05) |
| 2c | Re-géocodage fuzzy (`regeocod_fuzzy_v1.py`) | ✅ **Complété** — 194 acceptées, 6 pending |
| 3 | Géocodage manuel UI | ✅ **Complété** — 0 sondage sans géométrie |
| 4a | KED-HIER + RK + BLUP (5 params × 3 horizons) | ✅ **Complété** (06-05) |
| 4b | MTGP (8 params × 3 horizons) | ✅ **Complété** (06-05) |
| 4c | **KED-HIER V11 H2/H3 manquants** (cbr_95, gamma_d, w_opt) | 🔴 **À faire** — H2 et H3 manquants |
| 4d | **KED-HIER em_mpa/pl_mpa H2** | 🔴 **À faire** — H2 absent (H1 et H3 OK) |
| 5 | Mise à jour article scientifique | 🔄 **En cours** — révisions V2 définies |
| 6 | **Validation par blocs spatiaux** (NOUVEAU) | 🔴 **Artefact A1** — Requis par directeur |
| 7 | **PICP pour tous les paramètres** (NOUVEAU) | 🔴 **Artefact A2** — Requis par directeur |
| 8 | **Log-transform VBS avant krigeage** (NOUVEAU) | 🟡 **Artefact A5** — VBS asymétrie 2.41 |
| 9 | **Carte zones "prédites par géologie seule"** (NOUVEAU) | 🔴 **Artefact A9** — Kara/Savanes |

### 19.2 Analyse distribution spatiale — Résultats clés (2026-06-07)

Source : `docs/RECHERCHE/DISTRIBUTION_SPATIALE_SONDAGES.md`

| Indicateur | Valeur | Signification |
|------------|--------|---------------|
| Sondages GPS exact | 127 (22.2%) | ✅ Précis — utilisables pour validation locale |
| Sondages adm_random_cell | 445 (77.8%) | ⚠️ Approximatifs ± 5-20 km |
| Kara + Savanes | 59 (10.3%), **0 GPS** | 🔴 Entièrement prédit par géologie |
| Mailles vides | 29 017 (98.67%) | → Krigeage sur quasi-totalité du territoire |
| Max sondages/maille | 5 (Sagbado, Maritime) | ✅ Anomalie POINT(1,8.6) bien résolue |
| Canton le plus dense | Notse (54 sondages, 35 mailles) | ⚠️ Sur-représentation possible |

### 19.3 Artefacts à créer — Plan d'action issu de la revue du directeur

Tous ces artefacts sont détaillés dans :  
`docs/RECHERCHE/article_geostats_togo/POINTS_REVISION_PROCHAINE_ITERATION_V2.md`

#### Artefacts code (scripts Python)

| ID | Artefact | Chemin cible | Dépendances | Priorité |
|----|---------|-------------|-------------|----------|
| A1 | `validation_bloc_spatial.py` — LOO-CV par 5 blocs N-S | `scripts/` | DB port 5433, ai_interpolation_values | 🔴 |
| A2 | `validation_picp.py` — PICP pour tous paramètres | `scripts/` | A1 d'abord pour σ² krigeage | 🔴 |
| A5 | Flag `--log-transform` dans `run_ked_vbs_ip_wl_wp_horizons.py` | `scripts/` (modifier) | Existant | 🟡 |
| A8 | `validation_stationnarite.py` — Indice Moran I résidus | `scripts/` | scipy.stats, libpysal | 🟡 |

#### Artefacts données (à calculer après A1/A2)

| ID | Artefact | Format | Contenu |
|----|---------|--------|---------|
| A3 | Tableau RMSE par région | Markdown + CSV | 5 régions × 5 params × 3 horizons |
| A4 | Graphique PICP VBS avant/après log-transform | PNG 300dpi | Courbe couverture réelle vs nominale |

#### Artefacts QGIS / visualisation

| ID | Artefact | Chemin | Contenu |
|----|---------|--------|---------|
| A9 | Carte zones "prédites par géologie seule" | `exports_300dpi/` | Hachures sur Kara/Savanes |
| A6 | Tableau comparatif L1-L4 par paramètre | LaTeX + PNG | Couverture opérationnelle |

#### Artefacts manuscrit LaTeX

| ID | Section | Action | Fichier |
|----|---------|--------|---------|
| M1 | Abstract | Ajouter hétérogénéité nord/sud | `main.tex` |
| M2 | Matériel & Méthodes | Section validation par blocs | `main.tex` |
| M4 | Résultats | Tableau RMSE par région | `main.tex` |
| M5 | Résultats | Déplacer Em/Pl en Annexe | `main.tex` |
| M6 | Discussion | Effet lissant krigeage + Vertisols | `main.tex` |
| M8 | Conclusion | Checklist de Confiance Opérationnelle | `main.tex` |

### 19.4 Commandes pour terminer les calculs manquants (Phase 4c et 4d)

```powershell
# Phase 4c : KED-HIER cbr_95/gamma_d/w_opt H2 et H3
docker exec atlas-api-geo python3 /opt/atlas/scripts/run_ked_vbs_ip_wl_wp_horizons.py `
  --database-url "postgres://atlas:atlas@host.docker.internal:5433/atlas_clean" `
  --kinds cbr_95,gamma_d,w_opt `
  --hierarchical

# Phase 4d : KED-HIER em_mpa/pl_mpa H2 manquant
docker exec atlas-api-geo python3 /opt/atlas/scripts/run_ked_new_params_horizons.py `
  --database-url "postgres://atlas:atlas@host.docker.internal:5433/atlas_clean" `
  --kinds em_mpa,pl_mpa `
  --hierarchical
```

### 19.5 Squelettte du script A1 — Validation par blocs spatiaux

```python
# scripts/validation_bloc_spatial.py
"""
Validation par blocs spatiaux (5 blocs longitudinaux N-S, 120 km chacun).
Motivé par : Roberts et al. (2017) Ecography — LOO optimiste de 15-30%
             en présence de corrélation spatiale (portée > 50 km).
"""
import numpy as np
import psycopg2
import pandas as pd
...
# 5 blocs N-S : latitudes Togo 6.1°N à 11.1°N → tranches de 1°
BLOCS_LAT = [(6.1, 7.1), (7.1, 8.1), (8.1, 9.1), (9.1, 10.1), (10.1, 11.1)]
BLOC_NAMES = ['Maritime-sud', 'Maritime-nord/Plateaux-sud', 
               'Plateaux-central', 'Centrale/Kara-sud', 'Kara/Savanes']

def run_bloc_validation(conn, param: str, horizon: str):
    """LOO par bloc : entraîner sur 4 blocs, prédire sur le 5ème."""
    results = []
    for i, (lat_min, lat_max) in enumerate(BLOCS_LAT):
        # Masquer le bloc i
        train_mask = "NOT (lat BETWEEN %s AND %s)"
        test_mask  = "lat BETWEEN %s AND %s"
        # ... charger train/test, entraîner KED-H, prédire sur test ...
        # Calculer RMSE du bloc i
        results.append({
            'bloc': BLOC_NAMES[i], 'lat_range': f"{lat_min}-{lat_max}°N",
            'n_test': n_test, 'rmse_bloc': rmse_bloc
        })
...
## Section 20 — Mise à jour session 2026-06-07 (après-midi)

**Statut** : PHASE 4 EN COURS — Calculs ML lancés

### 20.1 Artefacts créés dans cette session

| Artefact | Statut | Résultat |
|---------|--------|---------|
| `atlas.v_contexte_geologique` (SQL) | ✅ Créé | 29 407 mailles, 14 pedo, 22 geo |
| `scripts/validation_bloc_spatial.py` | ✅ Créé | A1 — 5 blocs N-S, krigeage ordinaire |
| `scripts/validation_picp.py` | ✅ Créé | A2 — LOO-CV + PICP 90/95/99% |
| `scripts/validation_stationnarite.py` | ✅ Créé | A8 — Moran I + Levene N/S |
| `scripts/sgs_interpolation.py` | ✅ Créé | P1 — SGS gstools + log-transform |
| `scripts/generate_article_figures.py` | ✅ Créé | Figures 300 DPI |
| `--log-transform` flag KED | ✅ Ajouté | A5 — log(val+1) avant krigeage |
| RK portance étendu | ✅ Ajouté | P3 — cbr_95, gamma_d, w_opt |
| `MATRICE_CORRELATIONS_PARAMETRES.md` | ✅ Créé | M-6 — r(IP,WL)=0.804 |
| `METRIQUES_VALIDATION_COMPARATIVES.md` | ✅ Créé | A3 — LOO vs Bloc RMSE |
| `CHECKLIST_CONFIANCE_OPERATIONNELLE.md` | ✅ Créé | A6/A7/M8 |
| `figures/A9_sondage_density_map.png` | ✅ Généré | 300 DPI |
| `figures/EXTRA_rmse_loo_vs_bloc.png` | ✅ Généré | 300 DPI |

### 20.2 Calculs en cours (lancés à 14h10)

| Calcul | Statut | Résultat partiel |
|--------|--------|-----------------|
| KED Hiérarchique vbs/ip/wl/wp/eg H1-H3 | 🟡 En cours | — |
| RK SCORPAN cbr_95 H1 | 🟡 En cours | — |
| RK SCORPAN gamma_d H1 | 🟡 En cours | — |
| RK SCORPAN w_opt H1 | 🟡 En cours | — |
| PICP VBS/IP/EG H1 | 🟡 En cours | — |
| Validation bloc H2/H3 | 🟡 En cours | — |

### 20.3 Résultats obtenus

**Validation bloc-spatial H1 :**
- VBS: RMSE_bloc=**4.067** vs LOO=2.933 → +38.7% optimisme LOO ✅ Roberts 2017 confirmé
- IP: +10.1%, WL: +13.1%, WP: +9.5%, EG: +6.0%

**Stationnarité résidus :**
- VBS H1 : Moran I=-1.06, p=0.596 → stationnarité ✅
- IP H1 : Moran I=-1.27, p=0.448 → stationnarité ✅ (Levene borderline p=0.05)

**Matrice corrélations :**
- r(IP,WL)=0.804, r(IP,EG)=0.735, r(VBS,WP)=0.074 (WP quasi-indépendant)
- r(CBR,γd)=0.614, r(γd,w_opt)=-0.635 (logique Proctor confirmée)

*Mise à jour 2026-06-07 | v3.1 | Session calculs ML*
