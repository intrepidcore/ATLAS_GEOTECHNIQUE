...
|---|---|
| Sondages en base | **123** (l'audit annonçait 87 — import supplémentaire effectué) |
| Sondages avec maille_code | 114 (géolocalisés sur la grille 2 km²) |
| Horizons mesurés | H1 = 1.0 m · H2 = 1.5 m · H3 = 2.0 m |
| Paramètres mesurés | 7 (voir tableau 2.2) |
| Données Proctor (γd, wopt) | Présentes en base mais source incertaine |

### 2.2 Paramètres géotechniques disponibles

| Paramètre | Description | Plage physique | N mesures (H1) |
|---|---|---|---|
| **VBS** | Valeur au Bleu de Méthylène (activité argileuse) | 0 – 20 g/100g | 204 |
| **IP** | Indice de Plasticité (Atterberg) | 0 – 80 % | 220 |
| **WL** | Limite de Liquidité (Atterberg) | 20 – 120 % | 211 |
| **WP** | Limite de Plasticité (Atterberg) | 10 – 60 % | 202 |
| **EG** | Potentiel de Gonflement (`potentiel_gonflement` en DB) | 0 – 20 % | 186 |
| **Passant 2mm** | Tamisage granulométrique | — | via KED H1/H2/H3 |
| **Passant 80µm** | Fine fraction argileuse | — | via KED H1/H2/H3 |

> **Note importante pour l'ingénieur :** La colonne EG dans la vue `v_echantillons_essais` s'appelle `potentiel_gonflement` (pas `eg`). Tout script Python qui tente d'accéder à `eg` directement échouera.
...
| `atlas.zones_etude` | Table | 5 zones géologiques | 5 |
| `atlas.risque_gonflement` | Table | Classes de risque RGA (table de référence) | 342 |
...
## 4. Modèle L1 — Krigeage avec Dérive Externe (KED)

### 4.1 Principe

Le KED est une méthode géostatistique qui intègre une variable auxiliaire continue (ici : **l'altitude DSM COP30 à 30m**) comme fonction de dérive pour améliorer l'interpolation spatiale. Le modèle de variogramme **sphérique** a été retenu pour tous les paramètres.

**Formule :**
```
Z*(s₀) = Σ λᵢ Z(sᵢ)  sous contrainte  Σ λᵢ f(sᵢ) = f(s₀)
où f(s) = altitude DSM au point s
```

### 4.2 Covariables DSM utilisées

Toutes calculées à 29 407 mailles (couverture 100 %) :

| Feature | Description | Source | Méthode |
|---|---|---|---|
| `altitude_mean` | Altitude moyenne (m) | DSM COP30 30m | ST_Clip + ST_SummaryStats |
| `dem_slope_mean_deg` | Pente moyenne (degrés) | DSM COP30 | ST_Slope |
| `dem_tpi_mean` | Index de position topographique | DSM COP30 | Anneau 3-6 km |
| `dem_hand_mean` | Height Above Nearest Drainage | DSM COP30 | Zone 10 km |
| `distance_river_m` | Distance au cours d'eau le plus proche | OSM/DSM | ST_Distance |
...
```
Z*(s₀) = m̂(s₀) + ê*(s₀)
m̂(s₀) = Ridge(covariables SCORPAN à s₀)     # tendance
ê*(s₀) = Krigeage ordinaire des résidus      # composante stochastique
```

### 5.2 Covariables SCORPAN complètes

Le framework SCORPAN (Jenny 1941, McBratney 2003) modélise le sol comme fonction de :

| Facteur | Variable | Disponibilité |
|---|---|---|
| **S** — Substrat géologique | DSM : altitude, pente, TPI, HAND | ✅ 29 407 mailles |
| **C** — Climat | prec_annual, prec_dry, prec_wet (WorldClim 2.5') | ✅ 29 404/29 407 |
| **C** — Climat (saisonnalité) | bio4, bio12, bio15, bio17 (WorldClim BIO) | ✅ 29 407 mailles |
| **P** — Position | lon, lat, distance rivière | ✅ 29 407 mailles |

**Features utilisées dans le modèle Ridge :**
```python
