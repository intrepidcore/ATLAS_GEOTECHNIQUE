-- Migration 104 : Fix atlas.geocode_sondage() - ajoute la mise a jour de location_mode
-- Probleme : la fonction geocode correctement (maille_code, adm3_id, etc.) mais
--            ne met JAMAIS a jour location_mode. Resultat : 370 sondages avec
--            location_mode = NULL meme s'ils ont des coordonnees reelles.
-- Date : 2026-06-04
...
BEGIN
    -- Recupere la geometrie et le mode de localisation actuels
    ...
    -- Ne pas re-geocoder si le fallback est deja marque
    -- (evite d'ecraser une correction manuelle en cours)
    ...
    -- Determiner le location_mode selon la precision / etat actuel
    -- Regle :
    --   Si location_mode est deja un mode "fort" (exact/gps/manual/inferred), le conserver.
    --   Si location_mode est NULL ou 'geocoded', classifier selon la precision.
    --   Ne JAMAIS ecraser adm_random_cell (regle de verrouillage, AUDIT ?4.3).
    DECLARE
        v_new_loc_mode text;
    BEGIN
        ...
        ELSIF v_current_loc_mode = 'adm_random_cell' THEN
            v_new_loc_mode := 'adm_random_cell';    -- verrouille, ne pas modifier
        ...
        ELSIF v_loc_accuracy IS NOT NULL AND v_loc_accuracy < 100 THEN
            v_new_loc_mode := 'gps';
        ELSE
            v_new_loc_mode := 'geocoded';           -- mode par defaut (mieux que NULL)
        END IF;

        -- Mise a jour complete : maille + ADM + location_mode
...
-- (pas le fallback POINT(1 8.6), deja traite par migration 103)
UPDATE atlas.sondages
...
BEGIN
    ...
    RAISE NOTICE 'Sondages classifies geocoded : %', v_count;
END;
$$;
