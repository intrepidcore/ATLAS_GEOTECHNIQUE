...
use crate::surveys::{
    get_test_unit, validate_test, validate_togo_bounds, CreateSurveyResponse, GeocodeRequest,
    NewSurveyRequest,
};
use axum::{
    extract::{Path, State},
...
pub async fn geocode_survey(
    ...
    // Déterminer le mode
    let (geom_update, location_accuracy) =
        ...
        } else if let (Some(mode), Some(level), Some(name)) = (
            &payload.location_mode,
            &payload.adm_level,
            &payload.adm_name,
        ) {
            // Mode centroid ou random
            let func = if mode == "centroid" {
                "place_at_centroid"
            } else {
                "place_random_in_adm"
            };
