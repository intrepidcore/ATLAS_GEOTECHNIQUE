...
pub async fn create_survey_geotech(
    ...
    for essai_prof in &payload.essais_par_profondeur {
        let profondeur_bd =
            sqlx::types::BigDecimal::from_str(&essai_prof.profondeur_m.to_string()).unwrap();
        ...
        for mesure in &essai_prof.mesures {
            ...
            let unit = mesure
                .unit
                .as_deref()
                .unwrap_or_else(|| get_default_unit(&mesure.type_essai));

            let valeur_num_bd = mesure
                .valeur_numerique
    ...
    if let Some(ref classifs) = payload.classifications_par_profondeur {
        for classif_prof in classifs {
            let profondeur_bd =
                sqlx::types::BigDecimal::from_str(&classif_prof.profondeur_m.to_string()).unwrap();
    ...
    let maille_code: Option<String> =
        sqlx::query_scalar("SELECT maille_code FROM sondages WHERE id = $1")
            .bind(sondage_id)
            .fetch_one(&mut *tx)
            .await
            .ok();
...
pub async fn get_survey_geotech(
    ...
    let location_accuracy: String = sondage
        .try_get("location_accuracy")
        .unwrap_or_else(|_| "exact".to_string());
    let date: Option<String> = sondage
    ...
    let essais: Vec<EssaiOutput> = match essais_rows {
        Ok(rows) => rows
            ...
            .map(|r| {
                ...
                let val_num_bd: Option<sqlx::types::BigDecimal> =
                    r.try_get("valeur_numerique").ok().flatten();

                EssaiOutput {
                    id: r.try_get("id").unwrap_or_default(),
                    profondeur_m: depth_bd
