    ...
    match mode {
        GeolocationMode::Exact => {
            if row.lon.is_none() || row.lat.is_none() {
                return Err(anyhow!("Coordonnées lon/lat obligatoires pour mode 'exact'"));
            }
...
pub async fn check_duplicate_fingerprint(
    pool: &PgPool,
    fingerprint: &str,
) -> Result<bool> {
    let row = sqlx::query(
...
pub fn validate_analyse_qualitative(
    type_essai: &str,
    analyse: &str,
) -> Result<String> {
    ...
    if valid_values.iter().any(|&v| normalize_analyse(v) == normalized) {
        Ok(normalized)
    } else {
        Err(anyhow!(
    ...
    match type_essai {
        "qc" => {
            match (from_unit, to_unit) {
                ...
                _ => Err(anyhow!("Conversion {} -> {} non supportée", from_unit, to_unit)),
            }
        }
        "Proctor_gdmax" => {
            match (from_unit, to_unit) {
...
mod tests {
    ...
    fn test_convert_unit() {
        ...
        assert_eq!(convert_unit(2.0, "t/m³", "g/cm³", "Proctor_gdmax").unwrap(), 2.0);
    }
}
