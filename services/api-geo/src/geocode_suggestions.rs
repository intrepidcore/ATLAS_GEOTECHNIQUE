...
pub async fn accept_suggestion(
    ...
    let sondage_id = suggestion
        .entity_id
        .ok_or((StatusCode::BAD_REQUEST, "Missing entity_id".to_string()))?;

    let adm3_pcode = suggestion
        .top_code
        .ok_or((StatusCode::BAD_REQUEST, "Missing top_code".to_string()))?;

    // Récupérer l'ADM3
    let adm3_gid: i32 = sqlx::query_scalar("SELECT gid FROM adm3 WHERE adm3_pcode = $1")
        .bind(&adm3_pcode)
        .fetch_one(pool)
        .await
    ...
    let sondage_uuid = sondage_id
        .parse::<uuid::Uuid>()
        .map_err(|e| (StatusCode::BAD_REQUEST, format!("Invalid UUID: {}", e)))?;

    let old_values: (Option<String>, Option<String>, Option<String>) = sqlx::query_as(
        "SELECT location_mode::text, adm3_name, ST_AsText(geom) FROM public.sondages WHERE id = $1",
    )
    .bind(sondage_uuid)
    .fetch_one(pool)
    .await
    .map_err(|e| {
        (
            ...
            format!("Failed to fetch old values: {}", e),
        )
    })?;
    ...
    .map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Failed to update suggestion: {}", e),
        )
    })?;
...
pub async fn auto_geocode_suggestions(
    ...
    let result: serde_json::Value = sqlx::query_scalar("SELECT public.run_auto_geocode_batch($1)")
        .bind(threshold)
        .fetch_one(pool)
        .await
        .map_err(|e| {
    ...
    // Broadcaster l'événement WebSocket
    crate::websocket::broadcast_event(&state.ws_tx, crate::events::WsEvent::RefreshStats);
