...
pub async fn apply_staging_operation(
    ...
    match operation.op {
        RowOperation::Insert => {
            ...
            let values: Vec<String> = operation.data.values()
                .map(|v| value_to_sql_string(v))
                .collect();
        ...
        RowOperation::Update => {
            if let Some(row_id) = operation.row_id {
                let set_clauses: Vec<String> = operation.data.iter()
                    .map(|(k, v)| format!("\"{}\" = {}", k, value_to_sql_string(v)))
                    .collect();
                
                let query = format!(
                    "UPDATE {}.{} SET {}, _staging_op = 'UPDATE' WHERE id = $1",
                    schema_ident, staging_ident, set_clauses.join(", ")
                );
...
pub async fn validate_staging(
    ...
    let columns = super::schema::get_columns(
        pool,
        &staging_info.schema_name,
        &staging_info.table_name,
    ).await?;
    
    for column in columns.iter().filter(|c| !c.is_nullable && c.column_default.is_none()) {
        let schema_ident = sqlx::query_scalar::<_, String>("SELECT quote_ident($1)")
            .bind(&staging_info.schema_name)
            .fetch_one(pool)
            .await?;
        ...
        if null_count > 0 {
            errors.push(ValidationError {
                ...
                message: format!("La colonne '{}' ne peut pas être NULL ({} lignes)", column.name, null_count),
            });
        }
    }
    
    // Vérifier les géométries si applicable
    let geom_info = super::schema::get_geometry_info(
        pool,
        &staging_info.schema_name,
        &staging_info.table_name,
    ).await?;
...
pub async fn preview_staging(
    ...
    let columns = super::schema::get_columns(pool, &staging_info.schema_name, &staging_info.table_name).await?;
