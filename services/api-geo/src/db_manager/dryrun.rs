...
pub async fn dryrun_add_column(
    ...
    if !request.is_nullable {
        ...
        if request.default_value.is_none() {
            warnings.push("Colonne NOT NULL sans valeur par défaut : nécessite que la table soit vide".to_string());
        }
    }
    ...
    if row_count > 10_000_000 {
        warnings.push(format!("Table volumineuse ({} lignes) : opération peut être longue", row_count));
    }
