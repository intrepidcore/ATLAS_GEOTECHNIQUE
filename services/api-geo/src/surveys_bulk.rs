...
pub async fn bulk_import_surveys(
    ...
    let mut surveys_map: std::collections::HashMap<String, Vec<(usize, &BulkSurveyRow)>> = std::collections::HashMap::new();
    
    for (idx, row) in payload.rows.iter().enumerate() {
        let code = row.code.clone().unwrap_or_else(|| format!("BULK-{}", idx));
        surveys_map.entry(code).or_insert_with(Vec::new).push((idx, row));
    }
