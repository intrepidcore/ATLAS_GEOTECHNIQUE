...
# ---------------------------------------------------------------------------
# Invariant du CDC de simulation : aucune lecture de la base sans se prononcer
# sur le corpus. Le label de source distingue ensuite les ecritures.
import sys as _s
_FC = {"reel": "NOT s.is_synthetique",
       "synth": "s.is_synthetique",
       "mixte": "TRUE"}
_CORPUS = None
for _a in list(_s.argv[1:]):
    if _a in _FC:
        _CORPUS = _a
        _s.argv.remove(_a)
if _CORPUS is None:
    raise SystemExit("corpus obligatoire : reel | synth | mixte")
_FILTRE_CORPUS = _FC[_CORPUS]
SOURCE_LABEL_SIM = ("vfs_extract_spectral.py" if _CORPUS == "reel"
                    else "SIM_SYNTH_2026-09_" + _CORPUS)
print("CORPUS : %s | source_label : %s" % (_CORPUS.upper(), SOURCE_LABEL_SIM),
      flush=True)
# ---------------------------------------------------------------------------
