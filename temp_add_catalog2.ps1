...
VALUES
  ('vbs_rk_h1', 'geotech', 'interpolation', 'g/100g', true, true, true, now(), true, true),
  ...
  ('ip_rk_h1', 'geotech', 'interpolation', 'kN/m2', true, true, true, now(), true, true),
  ...
  ('wl_rk_h1', 'geotech', 'interpolation', '%', true, true, true, now(), true, true),
  ('wp_rk_h1', 'geotech', 'interpolation', '%', true, true, true, now(), true, true)
ON CONFLICT (parameter_id) DO NOTHING;
SELECT parameter_id FROM atlas.ai_parameter_catalog WHERE parameter_id LIKE '%_rk_%';"