-- Vue complète des essais avec tous les champs et classifications
CREATE OR REPLACE VIEW v_samples_complete AS
SELECT 
  e.id,
  e.sondage_id,
  ...
  jsonb_build_object(
    ...
    'gtr', CASE 
      WHEN e.passant_80um IS NULL THEN 'N/A'
      WHEN e.passant_80um < 35 THEN 'B'
      WHEN e.passant_80um < 70 THEN 'B/A'
      ELSE 'A'
    END
  ) AS classif
...
CREATE INDEX IF NOT EXISTS idx_samples_complete_sondage ON essais_geotechniques(sondage_id);
