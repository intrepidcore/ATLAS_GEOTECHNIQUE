...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '⏭️  Migration 008_create_analysis_views skippée: atlas.mailles introuvable.';
    RETURN;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '⏭️  Migration 008_create_analysis_views skippée: public.sondages introuvable.';
    RETURN;
  END IF;

  -- Tables d'essais optionnelles: la vue doit rester créable même si elles n'existent pas.
  EXECUTE $sql$
    ...
    LEFT JOIN essais_geotechniques eg ON s.id = eg.sondage_id
    LEFT JOIN essais_vbs vbs ON eg.echantillon_id = vbs.echantillon_id
    LEFT JOIN essais_atterberg att ON eg.echantillon_id = att.echantillon_id
...
-- Notes: les requêtes de test top-level sont volontairement omises pour éviter
-- de casser le bootstrap desktop. Elles peuvent être exécutées manuellement.
