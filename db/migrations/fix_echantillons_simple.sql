...
-- Date : 2025-11-07
-- Structure réelle: id, sondage_id, depth_m, date, laboratory, norm, rho_s_gcm3, water_content_w, is_index, eg, meta, created_at, updated_at
-- ============================================================================
...
ALTER TABLE echantillons ADD COLUMN depth_m_num NUMERIC;
...
ALTER TABLE echantillons ADD COLUMN date_date DATE;
...
UPDATE echantillons SET date = NULL WHERE lower(trim(date)) IN ('nan', '');
...
UPDATE echantillons SET
    ...
    depth_m_num = CASE WHEN depth_m ~ '^-?\d+\.?\d*$' THEN depth_m::numeric ELSE NULL END,
    ...
    date_date = CASE WHEN date ~ '^\d{4}-\d{2}-\d{2}' THEN date::date ELSE NULL END;
...
ALTER TABLE echantillons RENAME COLUMN depth_m TO depth_m_old;
...
ALTER TABLE echantillons RENAME COLUMN date TO date_old;
...
ALTER TABLE echantillons RENAME COLUMN depth_m_num TO depth_m;
...
ALTER TABLE echantillons RENAME COLUMN date_date TO date;
